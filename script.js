...
/*
    (function () {
      const ALLOWED_PIN = "9900"; // change this to your own PIN

      const pin = prompt("Enter PIN to access:");
      if (pin !== ALLOWED_PIN) {
        alert("Unauthorized");
        // send them away (any URL you like)
...
function createFormRow(prefill = {}) {
   ...
   errDate.className = 'inline-error';
   errDate.style.display = 'none';
   colDate.appendChild(lblDate);
   colDate.appendChild(inpDate);
   colDate.appendChild(errDate);

   const colSession = document.createElement('div');
   colSession.className = 'field';
   const lblSession = document.createElement('label');
   ...
   optAM.value = 'AM';
   optAM.textContent = 'AM';
   const optPM = document.createElement('option');
   optPM.value = 'PM';
   optPM.textContent = 'PM';
   selSession.appendChild(optEmpty);
   selSession.appendChild(optAM);
   selSession.appendChild(optPM);
   if (prefill.session) selSession.value = prefill.session;
   const errSession = document.createElement('span');
   errSession.className = 'inline-error';
   errSession.style.display = 'none';
   colSession.appendChild(lblSession);
   colSession.appendChild(selSession);
   colSession.appendChild(errSession);
   ...
   errQty.className = 'inline-error';
   errQty.style.display = 'none';
   colQty.appendChild(lblQty);
   colQty.appendChild(inpQty);
   colQty.appendChild(errQty);

   const colPct = document.createElement('div');
   colPct.className = 'field';
   const lblPct = document.createElement('label');
   lblPct.textContent = 'Percentage';
   ...
   const errPct = document.createElement('span');
   errPct.className = 'inline-error';
   errPct.style.display = 'none';
   colPct.appendChild(lblPct);
   colPct.appendChild(inpPct);
   colPct.appendChild(errPct);

   const colAmt = document.createElement('div');
   colAmt.className = 'field col-amount';
   ...
   colAmt.appendChild(inpAmt);

   const colAct = document.createElement('div');
   colAct.className = 'field col-actions';
   const btnRemove = document.createElement('button');
   btnRemove.type = 'button';
   btnRemove.className = 'ghost small';
   btnRemove.textContent = 'Remove';
   btnRemove.addEventListener('click', () => {
      wrapper.remove();
      updateFormSerials();
   });
   colAct.appendChild(document.createElement('div'));
   colAct.appendChild(btnRemove);
   ...
   inpPct.addEventListener('blur', () => {
      ...
      if (msg) {
         errPct.style.display = 'block';
         errPct.textContent = msg;
      } else {
         errPct.style.display = 'none';
      }
   });
   ...
   reader.onload = (e) => {
      ...
      for (let r = 1; r < aoa.length; r++) {
         ...
         if (eid || ed || es || eq || ep) {
            errors.push({
               row: r + 1,
               issues: [eid, ed, es, eq, ep].filter(Boolean).join(' | '),
               raw: obj
            });
            continue;
         }
         ...
         if (existingKeys.has(key) || batchKeys.has(key)) {
            errors.push({
               row: r + 1,
               issues: 'Duplicate entry (same Date, Session and User ID already exists).',
               raw: obj
            });
            continue;
         }
...
function gatherAndValidateAll() {
   ...
   for (const wrapper of nodes) {
      ...
      if (eDate) {
         f.errDate.style.display = 'block';
         f.errDate.textContent = eDate;
         hasError = true;
      } else f.errDate.style.display = 'none';
      if (eSess) {
         f.errSession.style.display = 'block';
         f.errSession.textContent = eSess;
         hasError = true;
      } else f.errSession.style.display = 'none';
      if (eId) {
         f.errId.style.display = 'block';
         f.errId.textContent = eId;
         hasError = true;
      } else f.errId.style.display = 'none';
      if (eQty) {
         f.errQty.style.display = 'block';
         f.errQty.textContent = eQty;
         hasError = true;
      } else f.errQty.style.display = 'none';
      if (ePct) {
         f.errPct.style.display = 'block';
         f.errPct.textContent = ePct;
         hasError = true;
      } else f.errPct.style.display = 'none';
...
function saveAllFormRows() {
   ...
   const savedRows = res.data.map(r => ({
      ...r,
      saved: true
   }));
...
function renderUserList() {
   ...
   ids.forEach(id => {
      ...
      tdName.textContent = USER_NAMES[id];
      tr.appendChild(tdId);
